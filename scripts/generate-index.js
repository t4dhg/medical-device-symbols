...
function cleanSvgContent(svgContent) {
  ...
  const cleaned = svgContent
    ...
    .trim()
    // Normalise hardcoded black so icons inherit the consumer's color via currentColor.
    // Only black is normalised: a future icon may legitimately need another fixed color.
    .replace(/(fill|stroke)="(#000000|#000|black)"/g, '$1="currentColor"')
    .replace(/\s+color="(#000000|#000|black)"/g, "");
...
let indexContent = `// AUTO-GENERATED by scripts/generate-index.js. DO NOT EDIT MANUALLY.
// To change an icon, edit its SVG in src/icons/ and run: npm run generate

import React from "react";

// Base icon component interface
export interface IconProps extends React.SVGProps<SVGSVGElement> {
  /** Width and height, as a pixel number or any CSS length. Defaults to 24. */
  size?: number | string;
  /**
   * Accessible label. When set, the icon renders with role="img" and this
   * value as its aria-label. Omit it for decorative icons and add aria-hidden.
   */
  title?: string;
}
